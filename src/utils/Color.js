...
export default class Color {
   ...
   */
  static getRandomColorHexBetween(ctx, color1, color2) {
    const parseColor = (color) => {
      try {
        ctx.fillStyle = color;
        const computed = ctx.fillStyle; // força normalização da cor

        // ctx.fillStyle sempre retorna no formato rgb(a)
        const match = computed.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*(\d*\.?\d+))?\)/);

        if (!match) throw new Error("Cor não pôde ser convertida.");
        ...
        console.warn(`Erro ao interpretar a cor: ${color}`, e);
        return [0, 0, 0, 1]; // fallback: preto
      }
    };
